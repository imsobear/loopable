...
export const REVIEW_FORMAT = [
  ...
  `}`,
  ``,
  `- "path" is exactly as the file is named in CHANGES.md.`,
  `- "line" is the number in the left column of CHANGES.md. Only lines that have`,
  `  a number there can be commented on; a line with no number was removed and`,
  `  cannot be. Do not count lines yourself.`,
  `- One finding per point, placed where a reader has to look rather than where`,
  `  you happened to notice it.`,
  `- "body" is markdown with no heading. Quote code without the line number and`,
  `  marker that CHANGES.md adds.`,
